...
# cifar_100 mean error at 5: 27.66%
class RMT(TTAMethod):
    def __init__(self, cfg, model, num_classes):
        ...
        self.contrast_mode = cfg.CONTRAST.MODE  # all
        self.temperature = cfg.CONTRAST.TEMPERATURE  # 0.1
        self.base_temperature = self.temperature  # 0.1
        self.projection_dim = cfg.CONTRAST.PROJECTION_DIM  # 128
        self.lambda_ce_src = cfg.RMT.LAMBDA_CE_SRC  # 1.0
        self.lambda_ce_trg = cfg.RMT.LAMBDA_CE_TRG  # 1.0
        self.lambda_cont = cfg.RMT.LAMBDA_CONT  # 1.0
        self.m_teacher_momentum = cfg.M_TEACHER.MOMENTUM  # 0.999
        # arguments neeeded for warm up
        self.warmup_steps = cfg.RMT.NUM_SAMPLES_WARM_UP // batch_size_src  # NUM_SAMPLES / batch_size
        self.final_lr = cfg.OPTIM.LR  # 1e-3
        ...
        if os.path.exists(fname):
            ...
            self.prototypes_src = torch.load(fname)  # dim = 640
        else:
        ...
        if self.warmup_steps > 0:
            ...
            self.warmup()  # "warmup" 的作用：1、模型在正式训练前先在源数据上训练1个epoch；2、在正式训练之前为模型提供一个较好的学习率初始值。（此处的作用使第一个)
    ...
    def forward_and_adapt(self, x):
        ...
        loss_entropy = self_training(x=outputs_test, x_aug=outputs_aug_test, x_ema=outputs_ema).mean(0)  # 对称蒸馏
