...
# import sys
# sys.path.insert(1,"/home/tjut_hanlei/test-time-adaptation-main")
# print(sys.path)
...
class RMT_REVISE(TTAMethod):
    def __init__(self, cfg, model, num_classes):
        ...
        self.e_margin = torch.tensor(math.log(self.num_classes) * 0.40).cuda()
        self.tta_transform = get_tta_transforms(self.dataset_name)

        # Setup EMA model
        self.model_ema = self.copy_model(self.model)
        for param in self.model_ema.parameters():
            param.detach_()  # 确保对 self.model 的梯度计算和参数更新不会影响到 self.model_ema
        ...
        if self.warmup_steps > 0:
            ...
            if os.path.exists(ckpt_path):
                ...
                self.warmup()  # "warmup" 的作用：1、模型在正式训练前先在训练集上训练几个epoch；2、在正式训练之前为模型提供一个较好的学习率初始值。（此处的作用使第一个)
    ...
    def contrastive_loss(self, features, labels=None, mask=None):
        ...
        # compute logits
        anchor_dot_contrast = torch.div(torch.matmul(anchor_feature, contrast_feature.T), self.temperature)  # 先内积，再执行元素级别的除法运算
        ...
        # compute log_prob
        exp_logits = torch.exp(logits) * logits_mask  # logits * logits_mask ：剔除自己与自己的cosine
        log_prob = logits - torch.log(exp_logits.sum(1, keepdim=True))  # exp_logits.sum(1, keepdim=True)：计算每个features与剩余599个features的cosine的和
    ...
    def forward_and_adapt(self, x):
        ...
        entropys = -(outputs_test.softmax(1) * outputs_test.log_softmax(1)).sum(1)
        filter_ids = torch.where(entropys < self.e_margin)
        features_test_, outputs_test_ = features_test[filter_ids], outputs_test[filter_ids]
        imgs_test_ = imgs_test[filter_ids]
        ...
        features_aug_test_ = features_aug_test[filter_ids]
        outputs_aug_test_ = outputs_aug_test[filter_ids]
        # forward original test data through the ema model
        outputs_ema = self.model_ema(imgs_test)
        outputs_ema_ = outputs_ema[filter_ids]
        with torch.no_grad():
            # dist[:, i] contains the distance from every source sample to one test sample
            dist = F.cosine_similarity(
                x1=self.prototypes_src.repeat(1, features_test_.shape[0], 1),
                x2=features_test_.view(1, features_test_.shape[0], features_test_.shape[1]).repeat(self.prototypes_src.shape[0], 1, 1),
                dim=-1)
        ...
        features = torch.cat([self.prototypes_src[indices],
                              features_test_.view(features_test_.shape[0], 1, features_test_.shape[1]),
                              features_aug_test_.view(features_test_.shape[0], 1, features_test_.shape[1])], dim=1)
        loss_contrastive = self.contrastive_loss(features=features, labels=None)  # 对比损失
        loss_entropy = self_training(x=outputs_test_, x_aug=outputs_aug_test_, x_ema=outputs_ema_, Threshold=self.e_margin).mean(0)  # 对称蒸馏
...
# @torch.jit.script
def self_training(x, x_aug, x_ema, Threshold):  # -> torch.Tensor:

    entropy_1 = -(x_ema.softmax(1) * x.log_softmax(1)).sum(1)
    coeff_1 = 1 / torch.exp(entropy_1.clone().detach() - Threshold)
    entropy_1 = entropy_1.mul(coeff_1)

    entropy_2 = -(x.softmax(1) * x_ema.log_softmax(1)).sum(1)
    coeff_2 = 1 / torch.exp(entropy_2.clone().detach() - Threshold)
    entropy_2 = entropy_2.mul(coeff_2)

    entropy_3 = -(x_ema.softmax(1) * x_aug.log_softmax(1)).sum(1)
    coeff_3 = 1 / torch.exp(entropy_3.clone().detach() - Threshold)
    entropy_3 = entropy_3.mul(coeff_3)

    entropy_4 = -(x_aug.softmax(1) * x_ema.log_softmax(1)).sum(1)
    coeff_4 = 1 / torch.exp(entropy_4.clone().detach() - Threshold)
    entropy_4 = entropy_4.mul(coeff_4)
    return 0.25 * entropy_1.mean(0) + 0.25 * entropy_2.mean(0) + 0.25 * entropy_3.mean(0) + 0.25 * entropy_4.mean(0)
