...
def offline(args, trloader, ext, classifier, num_classes=1000):
    if os.path.exists(args.offline+'/offline.pth'):
        data = torch.load(args.offline+'/offline.pth')
        return data
    ...
    torch.save((feat_ext_mean, feat_ext_variance, feat_ext_mean_categories, feat_ext_variance_categories), args.offline+'/offline.pth')
    return feat_ext_mean, feat_ext_variance, feat_ext_mean_categories, feat_ext_variance_categories


def offline_r(args, trloader, ext, classifier, num_classes=1000):
    if os.path.exists(args.offline+'/offline_r.pth'):
        data = torch.load(args.offline+'/offline_r.pth')
        return data
    ...
    with torch.no_grad():
        for batch_idx, (inputs, labels) in enumerate(trloader):
            ...
            for i in range(len(labels)):
                if indices_in_1k[labels[i]]==True:
                    t.append(labels[i])
                    l.append(inputs[0][i].unsqueeze(0))
            inputs = torch.cat(l,dim=0)
            feat = ext(inputs.cuda()) # N, D
            b, d = feat.shape
            labels = classifier(feat).argmax(dim=-1)
    ...
    torch.save((feat_ext_mean, feat_ext_variance, feat_ext_mean_categories, feat_ext_variance_categories), args.offline+'/offline_r.pth')
    return feat_ext_mean, feat_ext_variance, feat_ext_mean_categories, feat_ext_variance_categories



"""提取source domain data的features 和 protos"""
# def offline(args, trloader, ext, classifier, class_num=1000):
#     ext = torch.load("/data/tjut_m/OWTTT/imagenet/results/ckpt_AfterTrain.pth")
#     ext.eval()
