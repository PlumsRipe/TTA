...
from utils.prepare_dataset import prepare_transforms, create_dataloader, ImageNetCorruption, ImageNet_, \
    prepare_ood_test_data, prepare_ood_test_data_r
...
parser.add_argument('--dataroot', default='/data/HDD1/hanlei/data/imagenet2012/')
parser.add_argument('--workers', default=8, type=int)
parser.add_argument('--delta', default=0.1, type=float)
parser.add_argument('--resume', default='/home/tjut_hanlei/OWTTT_3000/imagenet/results/', help='directory of pretrained model')
"""******************************************************************************************************************************"""
...
parser.add_argument('--normal', default=True,
                    help='Is conduct normal training (without extracting features from the source domain data)?')
...
parser.add_argument('--co_scale', default=0.5, type=float, help='contrastive loss scale')
...
args.outf = f'/home/tjut_hanlei/OWTTT_3000/imagenet/results/log/{args.dataset}'
print(args)
print('\n')
...
if args.dataset == 'ImageNet-C':
    target_dataset_test = prepare_ood_test_data(args.dataroot, args.corruption, transform=val_corrupt_transform,
                                                is_carry_index=True, OOD=args.strong_OOD, OOD_transform=val_transform)
    class_num = 1000

elif args.dataset == 'ImageNet-R':
    indices_in_1k = [wnid in imagenet_r_wnids for wnid in all_wnids]
    target_dataset_test = prepare_ood_test_data_r(args.dataroot, args.corruption, transform=val_corrupt_transform,
                                                  is_carry_index=True, OOD=args.strong_OOD, OOD_transform=val_transform)
    class_num = 200
else:
...
for te_idx, (te_inputs, te_labels) in enumerate(target_dataloader_test):
    ...
    if seen_mask.sum().item() != 0:
        ...
        tmp_cov = ema_ext_total_cov.cuda() + alpha * (delta_pre.t() @ delta_pre - b * ema_ext_total_cov.cuda()) - delta[
                                                                                                                  :,
                                                                                                                  None] @ delta[
                                                                                                                          None,
                                                                                                                          :]
        ...
        if args.contrastive_loss:
            with torch.no_grad():
                dist = F.cosine_similarity(
                    ...
                    x2=F.normalize(feat_ext[seen_mask]).view(1, feat_ext[seen_mask].shape[0],
                                                             feat_ext[seen_mask].shape[1]).repeat(
                        net.fc.weight.data.shape[0], 1, 1),
                    dim=-1)
            ...
            features = torch.cat([net.fc.weight.data.unsqueeze(1)[indices],
                                  F.normalize(feat_ext[seen_mask]).view(feat_ext[seen_mask].shape[0], 1,
                                                                        feat_ext[seen_mask].shape[1]), ],
                                 dim=1)
...
if args.outf != None:
    ...
    # if args.save:
    #     torch.save(ext.state_dict(), "/home/tjut_hanlei/OWTTT_3000/imagenet/results/ImageNet-C/final_embedding.pth")
    #     torch.save(embedding, "/home/tjut_hanlei/OWTTT_3000/imagenet/results/ImageNet-C/protos_AfterTrain_embedding.pth")
